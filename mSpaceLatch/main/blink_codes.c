...
// The error_blink_task function is responsible for blinking the LED on the
// ESP32 board to indicate an error code. The function takes in an error code
// as a parameter and blinks the LED on the board to indicate the error code.
// The error code is the number of times the LED should blink in a row. If the
// error code is 0, then the LED will stay on indefinitely. If the error code
// is not 0, then the LED will blink the given number of times and then stop
// for 2 seconds before blinking again. The task will run indefinitely until
// the task is deleted by another task.
void error_blink_task(void *pvParameter) {
  // Cast the parameter to an integer and store it in a variable
  int error_code = *(int *)pvParameter;

  // Variable to indicate whether the task is running or not
  bool task_running = true;

  // Print the error code to the UART
  ESP_LOGI("blink_task", "Error code: %d", error_code);
  // app_buzzer_cfg();

  // If the error code is 0, turn the LED on and block indefinitely
  if (error_code == 0) {

    gpio_set_level(LED_ON, 1);

    while (task_running) {
      vTaskDelay(portMAX_DELAY); // task blocks indefinitely
    }
  }
  // If the error code is not 0, blink the LED and then stop for 2 seconds
  else {
    while (task_running) {
      // Blink the LED the number of times indicated by the error code
      for (int i = 0; i < error_code; i++) {
        gpio_set_level(LED_ON, 1);
        bz_tone(700, BLINK_DURATION); // beep takes BLINK_DURATION

        gpio_set_level(LED_ON, 0);
        vTaskDelay(BLINK_DURATION / portTICK_PERIOD_MS);
      }
      // Stop blinking for 2 seconds
...
// The update_error_code function is responsible for updating the error code
void update_error_code(void) {
  ESP_LOGI("Error code", "New error code: %d", error_code);
